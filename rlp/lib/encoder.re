/**
  Gives the Big Endian representation of a given length
  */
let rec to_binary = length =>
  if (length == 0) {
    "";
  } else {
    to_binary(length / 256) ++ String.make(1, Char.chr(length mod 256));
  };

let encode_length = (length, offset) =>
  if (length < 56) {
    Char.chr(length + offset) |> String.make(1);
...
let is_single_char = str =>
  String.length(str) == 1 && Char.code(str.[0]) <= 128;
